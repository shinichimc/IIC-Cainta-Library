...
if(isset($_GET['class_id'])){
  ...
  $_SESSION['category_name'] = $dbh->query("select class_name from class_list where class_id = ".$_GET['class_id'])->fetchColumn();
...
<div id="superwrapper">
        ...
        <div id="crumbs">
          <ul>
           ...
           <!--  <li><a href="#">Welcome Page</a></li> -->
            <li><a href="category_search.php">Category Search   <span class="icon-search"></span></a></li>
            <?php if(isset($_SESSION['category_name'])) : ?>
            <li><a href=""><?php echo $_SESSION['category_name'];?></a></li>
            <?php endif; ?>
          </ul>
        </div> <!-- ** breadcrumbs ** -->
      </div>
      <a href="category_search.php?clear=0" class="btn-refresh btn btn-default btn-sm" data-toggle="tooltip" data-placement="left" title="Clear Search"><span class="icon-arrows-cw" style="color:rgba(100,100,100,0.8);"></span></a>
    </div>
  </div>

  <div id="sidebar">
    <h4 class="icon-tags" style="margin-left:-10px;">Category</h4>
    <hr>
    <ul>
      <?php foreach($categories as $category) : ?>
      <?php $count_category = $dbh->query("select count(*) from book_class where class_id = ".$category['class_id'])->fetchColumn(); ?>
      <li class="category-list"><a class="category-link" href="category_search.php?class_id=<?php echo $category['class_id'];?>"><?php echo $category['class_name'];?>&nbsp;&nbsp;&nbsp;<span class="category-badge"><?php echo $count_category; ?></span>
      </a></li>
      <?php endforeach ; ?>
  ...
  <div id="main">
    ...
    <?php if(isset($_GET['class_id'])|| isset($_GET['page']) ||isset($_SESSION['secret'])) : ?>
      ...
      <div class="result-info-container-cs">
        <div class="result-info-box-cs">
          ...
          <?php if($_SESSION['total'] >= ITEMS_PER_PAGE) :?>
            <p><?php echo $offset + 1,"-".$to." of ".$_SESSION['total']." results found for the category '".$_SESSION['category_name']."'";?></p>
          <?php elseif($_SESSION['total'] < ITEMS_PER_PAGE && $_SESSION['total'] != 0) : ?>
           <p><?php echo $offset + 1,"-". ($_SESSION['total'])." of ".$_SESSION['total']." results found for the category '".$_SESSION['category_name']."'";?></p>
          <?php else :?>
      ...
      <?php else : ?>
      <h2 class="general icon-search"> Category Search</h2><hr>
